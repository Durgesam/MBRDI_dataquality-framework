...
class GetDistinctCount(BaseStats):
    """
    Calculates the number of distinct rows for each column.

    Returns 'dictionary' having key as column name and value as the total number of distinct values
    ...
    def run(self) -> dict:
        ...
        for column in self.columns:
            self.value[column] = main.cache.get_distinct_count(self.dataframe, column)
