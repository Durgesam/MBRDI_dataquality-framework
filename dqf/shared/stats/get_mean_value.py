...
class GetMeanValue(BaseStats):
    """
    Calculates the mean value in a column.

    Returns 'dictionary' having key as column name and value as the mean value in that column.
    ...
    def run(self) -> dict:
        ...
        for column in self.columns:
            if str(self.dataframe.schema[column].dataType) in ['IntegerType', 'LongType', 'FloatType']:
                self.value[column] = self.dataframe.agg({column: 'mean'}).collect()[0][0]
            else:
