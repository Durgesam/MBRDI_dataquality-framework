...
class EmptyStringDataGenerator(BaseGenerator):
    """
        class to generate data in parquet-format for the empty-string check
    ...
    def __init__(self, output_folder, spark_session):
        ...
        self.output_folder = self.output_folder + "check_empty_strings/"

    def generate_data(self):
        # Create test data set for check_empty_string
        num_rows = 100

        for check_counter, num_empty_strings in enumerate([0, 1, 4]):
            # Create random data with 2 columns
            data = [[random.randint(-10, 10), self.random_string()] for _ in range(num_rows)]
